...
void TaskManager::addTask(vector<string> fields, int line){
	...
	string name, slots = "1", accept = "", reject = "";
...
void TaskManager::graph(string file){
	...
	if (system(cmd.c_str()) == 0){
		...
		cerr << "Creation of flowchart file '" << pngFile << "' failed." << endl;
}
...
bool TaskManager::validate(){
	...
	for (auto t : tasks){
		...
		if (t.hasAccept()){
			...
			if (!validAccept)
				cerr << t.getName() << ": accept slot '"
				<< t.getAccept() << "' not found in list of tasks." << endl;
		}
		...
		if (t.hasReject()){
			...
			if (!validReject)
				cerr << t.getName() << ": reject slot '"
				<< t.getReject() << "' not found in list of tasks." << endl;
		}
